/*******************************************************************************
* File Name: heartrate_timer_12mhz.c
* Version 2.20
*
*  Description:
...
#include "heartrate_timer_12mhz.h"
...
/*******************************************************************************
* Function Name: heartrate_timer_12mhz_Start
********************************************************************************
*
* Summary:
...
void heartrate_timer_12mhz_Start(void) 
{
    ...
    heartrate_timer_12mhz_CLKEN |= heartrate_timer_12mhz_CLKEN_MASK;
	heartrate_timer_12mhz_CLKSTBY |= heartrate_timer_12mhz_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: heartrate_timer_12mhz_Stop
********************************************************************************
*
* Summary:
...
void heartrate_timer_12mhz_Stop(void) 
{
    ...
    heartrate_timer_12mhz_CLKEN &= (uint8)(~heartrate_timer_12mhz_CLKEN_MASK);
	heartrate_timer_12mhz_CLKSTBY &= (uint8)(~heartrate_timer_12mhz_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: heartrate_timer_12mhz_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((heartrate_timer_12mhz_CLKEN & heartrate_timer_12mhz_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = heartrate_timer_12mhz_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = heartrate_timer_12mhz_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* heartrate_timer_12mhz__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(heartrate_timer_12mhz_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(heartrate_timer_12mhz_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: heartrate_timer_12mhz_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: heartrate_timer_12mhz_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = heartrate_timer_12mhz_GetSourceRegister();
    uint16 oldDivider = heartrate_timer_12mhz_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = heartrate_timer_12mhz_CLKEN & heartrate_timer_12mhz_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(heartrate_timer_12mhz_DIV_PTR, clkDivider);
                heartrate_timer_12mhz_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                heartrate_timer_12mhz_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(heartrate_timer_12mhz_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((heartrate_timer_12mhz_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(heartrate_timer_12mhz_DIV_PTR, clkDivider);
				heartrate_timer_12mhz_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: heartrate_timer_12mhz_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(heartrate_timer_12mhz_DIV_PTR);
}


/*******************************************************************************
* Function Name: heartrate_timer_12mhz_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    heartrate_timer_12mhz_MOD_SRC |= modeBitMask & (uint8)heartrate_timer_12mhz_MODE_MASK;
}


/*******************************************************************************
* Function Name: heartrate_timer_12mhz_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    heartrate_timer_12mhz_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(heartrate_timer_12mhz_MODE_MASK));
}


/*******************************************************************************
* Function Name: heartrate_timer_12mhz_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return heartrate_timer_12mhz_MOD_SRC & (uint8)(heartrate_timer_12mhz_MODE_MASK);
}


/*******************************************************************************
* Function Name: heartrate_timer_12mhz_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = heartrate_timer_12mhz_GetDividerRegister();
    uint8 oldSrc = heartrate_timer_12mhz_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        heartrate_timer_12mhz_MOD_SRC |= CYCLK_SSS;
        heartrate_timer_12mhz_MOD_SRC =
            (heartrate_timer_12mhz_MOD_SRC & (uint8)(~heartrate_timer_12mhz_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: heartrate_timer_12mhz_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return heartrate_timer_12mhz_MOD_SRC & heartrate_timer_12mhz_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: heartrate_timer_12mhz_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    heartrate_timer_12mhz_PHASE = clkPhase & heartrate_timer_12mhz_PHASE_MASK;
}


/*******************************************************************************
* Function Name: heartrate_timer_12mhz_GetPhase
********************************************************************************
*
* Summary:
...
{
    return heartrate_timer_12mhz_PHASE & heartrate_timer_12mhz_PHASE_MASK;
}
